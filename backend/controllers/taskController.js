...
exports.updateTaskStatus = async (req, res) => {
  try {
    ...
    // Synchronize linked enquiry if this task was converted from an enquiry
    try {
      if (status === 'Completed') {
        await Enquiry.updateMany({ convertedTask: task._id }, { $set: { status: 'Closed' } });
      } else if (status === 'In Progress') {
        await Enquiry.updateMany({ convertedTask: task._id, status: { $in: ['New', 'In Discussion', 'Converted'] } }, { $set: { status: 'In Discussion' } });
      }
    } catch (syncErr) {
      console.warn('Enquiry status sync warning:', syncErr.message);
    }
