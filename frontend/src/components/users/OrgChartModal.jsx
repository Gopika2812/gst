      ...
      <div className="relative w-full max-w-5xl rounded-3xl bg-white p-6 shadow-2xl border border-slate-100 max-h-[95vh] overflow-y-auto">
        ...
        <div className="py-8 px-4 flex flex-col items-center bg-gradient-to-b from-slate-50/50 via-white to-slate-50/80 rounded-2xl my-4 border border-slate-200/60 shadow-inner min-w-[700px]">
          ...
          <div className="flex flex-col items-center relative group">
            <div className="glacier-card relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#0F2B48] to-[#1E4976] p-4 text-white shadow-lg border-2 border-[#52A636] w-64 text-center">
              <div className="absolute top-2 right-2">
                <span className="rounded-full bg-[#52A636] px-2 py-0.5 text-[9px] font-extrabold text-white uppercase shadow-xs">
                  SUPER ADMIN
                </span>
              </div>
              <div className="mx-auto mb-2 flex h-12 w-12 items-center justify-center rounded-xl bg-white/10 text-lg font-bold backdrop-blur-md border border-white/20">
                {sainath.name ? sainath.name[0] : 'S'}
              </div>
              <h3 className="text-base font-extrabold text-white">{sainath.name}</h3>
              <p className="text-xs font-semibold text-emerald-300 mt-0.5">({sainath.designation || 'Founder & MD'})</p>
              
              {onAssignTask && (
                <button
                  onClick={() => onAssignTask(sainath)}
                  className="mt-3 w-full flex items-center justify-center space-x-1 rounded-lg bg-[#52A636] px-3 py-1.5 text-[11px] font-bold text-white shadow-xs hover:bg-[#438A2B] transition"
                >
